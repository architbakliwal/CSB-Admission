...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->iDB = $this->load->database('default', true);
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->iDB->select('password, salt')
		                  ->where('id', $id)
		                  ->limit(1)
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			$query = $this->iDB->select($this->identity_column)
			...
			$this->trigger_events('extra_where');
			$this->iDB->update($this->tables['users'], $data, array($this->identity_column => $identity));
		}
		else
		{
			$data = array(
			...
			$this->trigger_events('extra_where');
			$this->iDB->update($this->tables['users'], $data, array('id' => $id));
		}


		$return = $this->iDB->affected_rows() == 1;
		if ($return)
		{
	...
	public function clear_forgotten_password_code($code) {
		...
		$this->iDB->where('forgotten_password_code', $code);

		if ($this->iDB->count_all_results($this->tables['users']) > 0)
		{
			$data = array(
			...
			$this->iDB->update($this->tables['users'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->iDB->select('id, password, salt')
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->iDB->update($this->tables['users'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->iDB->where('username', $username)
	...
	public function email_check($email = '')
	{
		...
		return $this->iDB->where('email', $email)
		                ->count_all_results($this->tables['users']) > 0;
	}
	...
	public function identity_check($identity = '')
	{
		...
		return $this->iDB->where($this->identity_column, $identity)
		                ->count_all_results($this->tables['users']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->iDB->update($this->tables['users'], $update, array($this->identity_column => $identity));
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->iDB->insert($this->tables['users'], $user_data);

		$id = $this->iDB->insert_id();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->iDB->select($this->identity_column . ', username, email, id, password, active, last_login')
		                  ->where($this->identity_column, $this->iDB->escape_str($identity))
		                  ->limit(1)
		                  ->get($this->tables['users']);
	...
	function get_attempts_num($identity)
	{
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
            $this->iDB->select('1', FALSE);
            if ($this->config->item('track_login_ip_address', 'ion_auth')) $this->iDB->where('ip_address', $ip_address);
            else if (strlen($identity) > 0) $this->iDB->or_where('login', $identity);
            $qres = $this->iDB->get($this->tables['login_attempts']);
	...
	public function get_last_attempt_time($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->iDB->select_max('time');
            if ($this->config->item('track_login_ip_address', 'ion_auth')) $this->iDB->where('ip_address', $ip_address);
			else if (strlen($identity) > 0) $this->iDB->or_where('login', $identity);
			$qres = $this->iDB->get($this->tables['login_attempts'], 1);
	...
	public function increase_login_attempts($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			return $this->iDB->insert($this->tables['login_attempts'], array('ip_address' => $ip_address, 'login' => $identity, 'time' => time()));
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $expire_period = 86400) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->iDB->where(array('ip_address' => $ip_address, 'login' => $identity));
			// Purge obsolete login attempts
			$this->iDB->or_where('time <', time() - $expire_period, FALSE);

			return $this->iDB->delete($this->tables['login_attempts']);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->iDB->where_in($this->tables['users_groups'].'.'.$this->join['groups'], $groups);
			}
		}
		...
		{
			$this->iDB->limit($this->_ion_limit, $this->_ion_offset);
		...
		{
			$this->iDB->limit($this->_ion_limit);
		...
		{
			$this->iDB->order_by($this->_ion_order_by, $this->_ion_order);

			$this->_ion_order    = NULL;
			$this->_ion_order_by = NULL;
		}

		$this->response = $this->iDB->get($this->tables['users']);

		/*$this->response = $this->iDB->query("SELECT `dbapp_users`.*, `dbapp_users`.`id` as id, `dbapp_users`.`id` as user_id
		FROM (`dbapp_users`)
		WHERE `dbapp_users`.`id` =  '1'
		LIMIT 1");*/
	...
	public function get_users_groups($id=FALSE)
	{
		...
		//return $this->iDB->query("SELECT `dbapp_users_groups`.`group_id` as id, `dbapp_groups`.`name`, `dbapp_groups`.`description` FROM (`dbapp_users_groups`) JOIN `dbapp_groups` ON `dbapp_users_groups`.`group_id`=`dbapp_groups`.`id` WHERE `dbapp_users_groups`.`user_id` = '$id'");

		return $this->iDB->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		if ($this->iDB->where(array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id))->get($this->tables['users_groups'])->num_rows()) return false;

		if ($return = $this->iDB->insert($this->tables['users_groups'], array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id)))
		{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			{
				$this->iDB->delete($this->tables['users_groups'], array($this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id));
		...
		{
			if ($return = $this->iDB->delete($this->tables['users_groups'], array($this->join['users'] => (int)$user_id))) {
	...
	public function groups()
	{
		...
		$this->response = $this->iDB->get($this->tables['groups']);
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->iDB->where($this->tables['groups'].'.id', $id);
		}
	...
	public function update($id, array $data)
	{
		...
		$this->iDB->trans_begin();

		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column])
		{
			$this->iDB->trans_rollback();
		...
		$this->trigger_events('extra_where');
		$this->iDB->update($this->tables['users'], $data, array('id' => $user->id));

		if ($this->iDB->trans_status() === FALSE)
		{
			$this->iDB->trans_rollback();
	...
	**/
	...
	public function update_last_login($id)
	{
		...
		$this->iDB->update($this->tables['users'], array('last_login' => time()), array('id' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->iDB->update($this->tables['users'], array('remember_code' => $salt), array('id' => $id));

		if ($this->iDB->affected_rows() > -1)
		{
			// if the user_expire is set to zero we'll set the expiration two years from now.
			if($this->config->item('user_expire', 'ion_auth') === 0)
			{
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->iDB->select($this->identity_column.', id, username, email, last_login')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->iDB->get_where($this->tables['groups'], array('name' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		$this->iDB->insert($this->tables['groups'], $data);
		$group_id = $this->iDB->insert_id();
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->iDB->get_where($this->tables['groups'], array('name' => $group_name))->row();
		...
		$this->iDB->update($this->tables['groups'], $data, array('id' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		$this->iDB->delete($this->tables['users_groups'], array($this->join['groups'] => $group_id));
		// remove the group itself
		$this->iDB->delete($this->tables['groups'], array('id' => $group_id));
	...
	{
		$filtered_data = array();
		$columns = $this->iDB->list_fields($table);
	...
	protected function _prepare_ip($ip_address) {
		if ($this->iDB->platform() === 'postgre' || $this->iDB->platform() === 'sqlsrv' || $this->iDB->platform() === 'mssql')
		{
			return $ip_address;
		}
		else
